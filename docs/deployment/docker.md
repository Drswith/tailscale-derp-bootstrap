...
宿主机须在[支持范围](../verification.md)，并有固定公网 IPv4。Docker Engine、Compose v2 缺失时，脚本在 Ubuntu 使用系统 `docker.io`/`docker-compose-v2`，在 Debian 使用 Docker 官方软件源自动安装。仅当需要在本机构建镜像且缺 Buildx 时，脚本才补装 Buildx。现有 Docker daemon 不可用时，脚本会尝试通过 systemd 恢复服务和 socket。
