...
2. Choose one mode. The guided `bootstrap.sh` command in README obtains the complete project, prepares configuration and checks local installation. For manual control, native uses `install.sh` and a host `config.env`; Docker uses `docker/deploy.sh` and `docker/config.env`. Keep their ports and state separate. Check `versions.lock` and the matching example config before supplying commands.
...
5. Use the guided bootstrap when a one-line server installation is requested. It runs installation and local checks, and stores the DERP map fragment; interactive login still needs device authorization. With manual control, run the mode's `preflight`, then `install`, then `check` and `derpmap`. Interactive Docker login needs `docker/deploy.sh logs` and device authorization before `check`. On minimal hosts, `install` can add base packages that `preflight` needs.
